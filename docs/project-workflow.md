# Healthcare Capacity Intelligence Platform — How This Project Works

*A plain-English guide so you always know what we're doing and why.*
...
teach the computer to predict the future from it, put those predictions behind
a small web service, and show the results on dashboards — and we do every step
**in the open, on screen, one piece at a time.**
...
- **How many new referrals are coming?** (demand)
- **How long will people wait?** (waiting time)
- **Which hospital departments are about to miss the 18-week target?** (breach risk)
...
6. **Keep track of the models (registry).** We save each trained model with a
   note of when it was trained, how accurate it was, and what data it used — so
   we can always go back and compare.

7. **Serve the predictions (API).** A small web service with simple doorways
   ("give me the demand forecast for Cardiology") so other tools can ask for a
   prediction and get an answer in under two seconds.

8. **Watch it stay healthy (monitoring).** We keep an eye on whether the data
   is still arriving and whether the models are still accurate over time
   (models can drift, meaning they slowly get less accurate as the world
   changes).

9. **Show the results (dashboards).** The clean Gold tables feed Power BI, where
   you build the executive charts: total waiting, predicted demand, breach
   heatmaps, and a model-health page.

Everything runs on your own machine first (with the database and web service in
containers — self-contained boxes that run the same everywhere). The code is
written so it can move to Amazon's cloud later by changing settings, not code.
...
flowchart TD
    ...
    I2 --> J
    I3 --> J
    J --> K[Web service / API<br/>serves predictions]
    G --> L[Power BI dashboards]
    J --> M[Monitoring:<br/>data fresh? models still accurate?]
    K --> M
...
classDiagram
    ...
    class PredictionAPI {
        ...
        +predict_breach_risk()
        +health()
        responsibility: serve predictions fast
    }
    class Monitoring {
    ...
    class BILayer {
        ...
        +breach_heatmap()
        responsibility: show results to people
    }

    Ingestion --> Validation : raw data
    Validation --> Warehouse : cleaned data
    Warehouse --> FeatureStore : business tables
    Warehouse --> BILayer : data for charts
    FeatureStore --> ModelTraining : features
    ModelTraining --> ModelRegistry : trained models + scores
    ModelRegistry --> PredictionAPI : best model to serve
    PredictionAPI --> Monitoring : prediction quality
    Warehouse --> Monitoring : data quality
    Monitoring --> BILayer : health page
```
...
- A clean repo with notebooks (the tour) **and** tidy code files (the product).
- Real NHS data flowing all the way through to three working predictions.
- A small web service you can call for a prediction.
- Power BI dashboards on top.
- Tests and an automated check that runs when code changes.
